...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Existing image path from BE (e.g. "/uploads/products/xxx.jpg")
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImageFile(file);
    if (file) {
      setExistingImage(null); // new file replaces existing
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      await update({ id, body: fd }).unwrap();
    ...
    return (
      <AdminGuard>
        <div className="text-center py-20">
          ...
          <Link href={paths.admin.products} className="text-blue-600 hover:underline text-sm">
            ← Back to Products
          </Link>
        </div>
      </AdminGuard>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" min="0" {...register("stock")} className={inputCls(!!errors.stock)} />
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 mb-1.5">
              ...
              <span className="text-gray-400 font-normal">
                {existingImage ? "(current image shown — upload a new file to replace)" : "(jpg, png, webp — max 5 MB)"}
              </span>
            </label>
            <input
              ref={fileInputRef}
