...
const NAV = [
  { href: paths.dashboard, label: "Dashboard",  icon: "📊" },
  { href: paths.orders,    label: "My Orders",   icon: "📦" },
  { href: paths.cart,      label: "My Cart",     icon: "🛒" },
];
...
function Sidebar() {
  ...
  return (
    <aside className="w-60 flex-shrink-0 bg-[#1e2a4a] flex flex-col h-full">
      {/* Logo */}
      ...
      <nav className="flex-1 p-3 space-y-1">
          ...
          return (
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                active ? "bg-blue-600 text-white" : "text-blue-100 hover:bg-white/10"
              }`}>
        ...
        <Link href={paths.home}
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-blue-100 hover:bg-white/10 transition-colors">
...
function TopBar() {
  ...
  return (
    <header className="h-14 bg-white border-b border-gray-100 flex items-center justify-between px-6 flex-shrink-0">
      <h1 className="text-base font-semibold text-gray-800">My Account</h1>
