      ...
      <div className="max-w-7xl mx-auto">
        <SlideUp className="flex items-center justify-between mb-5">
          ...
          <motion.div whileHover={{ x: 4 }} transition={springSnappy}>
            <Link href={`${paths.products}?category=Daily+Essentials`} className="flex items-center gap-1 text-sm font-medium text-gray-500 hover:text-blue-600 transition-colors">
        ...
        {isLoading ? (
          ...
          <StaggerContainer className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4" stagger={0.07}>
            {products.map((p) => {
              const imgUrl = getImageUrl(p.image);
              return (
                <StaggerItem key={p.id}>
                  <motion.div whileHover={{ y: -5, scale: 1.03 }} whileTap={{ scale: 0.97 }} transition={springSnappy}>
                    <Link href={paths.product(p.id)} className="flex flex-col bg-amber-50 border border-amber-100 rounded-2xl p-4 hover:shadow-xl transition-shadow group items-center text-center">
